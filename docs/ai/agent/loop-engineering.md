...
我之前在 [AI Agent 核心概念](./agent-basis.md) 里讲过，它适合处理路径不确定、需要根据证据调整方向的任务。查线上故障、读代码库、排查测试失败，都属于这一类。
...
比如“生成初稿 → 审核 → 不通过就修改 → 再审核”，这本来就是 Graph 里的条件边和回边。我之前在 [AI 工作流中的 Workflow、Graph 与 Loop](./workflow-graph-loop.md) 里讲过，可靠的 Loop 至少要写清三件事：
...
我之前在 [Harness Engineering](./harness-engineering.md) 里用过一个说法：Agent = Model + Harness。模型负责推理和生成，Harness 负责环境、工具、反馈、沙箱、权限、观测和恢复。
