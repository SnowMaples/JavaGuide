...
**⚠️注意**：为防止恶意攻击者利用随机参数频繁查询不存在的数据导致冷库被击穿，可以在缓存层前置**布隆过滤器（Bloom Filter）**或在缓存中设置**空值占位符**，避免恶意请求穿透到冷库。详细介绍参考 [Redis 常见面试题总结(下)](../database/redis/redis-questions-02.md)（Redis 事务、性能优化、生产问题、集群、使用规范等）。
...
**任务调度迁移**是最常用的方案，可借助 XXL-Job、Elastic-Job 等分布式任务调度平台实现。关于任务调度的方案，我也写过文章详细介绍，可以查看这篇文章：[Java 定时任务详解](../system-design/schedule-task.md) 。
