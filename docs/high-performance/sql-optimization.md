...
详细介绍可以阅读这篇文章：[深度分页介绍及优化建议](./deep-pagination-optimization.md)。
...
MySQL 时间类型选择的详细介绍请看这篇：[MySQL 时间类型数据存储建议](../database/mysql/some-thoughts-on-database-storage-time.md)。
...
`BigDecimal`的详细介绍请参考这篇：[BigDecimal 详解](../java/basis/bigdecimal.md)。
...
> **推荐阅读**：[MySQL 执行计划分析](../database/mysql/mysql-query-execution-plan.md) 详细介绍了 EXPLAIN 各列的含义（id、select_type、type、key、rows、Extra 等），包括 MySQL 8.0.18+ 新增的 `EXPLAIN ANALYZE` 实际执行分析功能。另外，阿里的 [慢 SQL 治理经验总结](https://mp.weixin.qq.com/s/LZRSQJufGRpRw6u4h_Uyww) 也总结得不错。
...
详细介绍：[MySQL索引失效场景总结](../database/mysql/mysql-index-invalidation.md)。
